...
export default function StoryModal({
	...
	const displayData = viewMode
		? {
				...
				perplexityScore: historyData?.score_perplexity || 0,
		...
		: {
				...
				perplexityScore: cardStore.perplexityScore || 0,
	...
	const handleStarClick = async (rating) => {
		...
		try {
			...
			if (success) {
				...
				if (historyData) {
					historyData.score_human = rating;
				}
			}
		} catch (error) {
			showToast({
...
function EvaluationSection({
	...
	return (
		<>
			...
			>
				<Text>
					Skor Perplexity: {displayData.perplexityScore.toFixed(2)}
				</Text>
